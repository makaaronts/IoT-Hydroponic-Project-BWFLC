//
//  SoilHumidityBottomDataView.swift
//  Water Agri BWFLC
//
...
struct SoilHumidityBottomDataView: View {
    @State var data: [Feed] = []
    var body: some View {
        NavigationStack {
            List {
                Text("Bottom")
                    ...
                    Chart {
                        ForEach(data, id: \.entryId) { feed in
                            LineMark(x: .value(feed.createdAt, feed.createdAt), y: .value("%", Int(feed.field3) ?? 0))
                        }
                    }
                .frame(height: 250)
...
struct FeedD_ata: Decodable {
    let feeds: [F_eed]
}

struct F_eed: Decodable, Identifiable {
    let id = UUID()
    let entryId: Int
